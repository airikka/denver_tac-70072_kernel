...
//#if defined (CONFIG_Q8)    //Q8 Tp 
//#define WAKE_PORT			RK2928_PIN0_PD3
//#elif defined (CONFIG_86V)               //86V
#define WAKE_PORT		    RK2928_PIN3_PC1
//#endif
...
{
	if(gpio_request(IRQ_PORT,"IRQ_PORT") != 0){
		gpio_free(IRQ_PORT);
	...
	if(gpio_request(WAKE_PORT,"WAKE_PORT") != 0){
		gpio_free(WAKE_PORT);
		printk("zhongchu_init_platform_hw gpio_request error\n");
		return -EIO;
	}
