...
#define RK29_SDMMC_VERSION "Ver.5.01 The last modify date is 2012-12-01"
...
static inline void rk29_sdmmc_init_ocr(struct rk29_sdmmc *host)
{
	host->vcc = regulator_get(mmc_dev(host->mmc), host->supply_name);
	
    xbwprintk(5, "%d..%s: vcc-addr=0x%x [%s]\n", __LINE__, __FUNCTION__,\
                host->vcc,host->dma_name);

	if (IS_ERR(host->vcc)) {
		host->vcc = NULL;
	} else {
		host->mmc->ocr_avail = mmc_regulator_get_ocrmask(host->vcc);
		if (host->pdata && host->pdata->host_ocr_avail)
			dev_warn(mmc_dev(host->mmc),
				"pdata->host_ocr_avail will not be used\n");
	}

	if (host->vcc == NULL) {
		/* fall-back to platform data */
		if (host->pdata && host->pdata->host_ocr_avail)
			host->mmc->ocr_avail = host->pdata->host_ocr_avail;
		else
...
static inline void rk29_sdmmc_set_power(struct rk29_sdmmc *host,
				    unsigned char power_mode,
				    unsigned int vdd)
{
	if (host->vcc) 
	{
	    xbwprintk(5, "%d..%s: power-mode=%d, vdd=%d [%s]\n", __LINE__, __FUNCTION__,\
	                power_mode, vdd,host->dma_name);
        ...
        switch (ios->power_mode) 
        {
            case MMC_POWER_UP:
            	...
            	if(RK29_CTRL_SDMMC_ID == host->pdev->id)
            	{
                    ...
                    {
                        //if your porject does not have SDMMC0_PWR_PIN£¬then you have to control PMU.
                        rk29_sdmmc_set_power(host, ios->power_mode, ios->vdd);
                    }
...
static int rk29_sdmmc_probe(struct platform_device *pdev)
{
   ...
   if(RK29_CTRL_SDMMC_ID == host->pdev->id)   
	...
	if(pdata->setpower)
	{
        pdata->setpower(NULL, (char *)&host->supply_name[0]);  
    }
    rk29_sdmmc_init_ocr(host);
