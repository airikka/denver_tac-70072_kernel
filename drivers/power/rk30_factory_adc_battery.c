...
static int batt_table[2*BATT_NUM+6] =
{
	...
	3526, 3548, 3599, 3626, 3655, 3697, 3751, 3812, 3877, 3949, 4030,  //discharge
...
struct rk30_adc_battery_data {
	...
	struct delayed_work       delay_poweroff;
...
static int load_battery_file = -1 ;
	...
	//charging 	    
		if( is_charge_ok(bat)  ==  INVALID_CHARGE_CHECK){
			...
			if (is_charge_ok(bat) != CHARGE_IS_OK ){
				...
				if ((bat->full_times >= NUM_CHARGE_FULL_DELAY_TIMES) && (bat->bat_capacity >= 97)){
					if (bat->bat_status != POWER_SUPPLY_STATUS_FULL){
						bat->bat_status = POWER_SUPPLY_STATUS_FULL;
						rk_stop_charge(bat);
...
static void rk30_adc_battery_poweron_capacity_check(struct rk30_adc_battery_data *bat)
{
	...
	printk("%s: %d: >>>> new_capacity = %d\n",__FUNCTION__, __LINE__,new_capacity);
//#if  defined (CONFIG_BATTERY_RK30_USB_CHARGE)
...
//#endif
	...
	load_battery_file = 1 ;
      ...
      printk("%s: %d: >>>>  old_capacity = %d\n",__FUNCTION__, __LINE__,old_capacity);
	...
	//chargeing state
      ...
      printk("%s: %d: >>>> is_reboot_charging = %d\n",__FUNCTION__, __LINE__,bat->pdata->is_reboot_charging);
		if( bat  ->pdata->is_reboot_charging == 1)
			bat ->bat_capacity = (old_capacity < 10) ?(old_capacity+2):old_capacity;
		else
			bat ->bat_capacity = (new_capacity < old_capacity - 25) ? new_capacity : old_capacity;
	}else{
		if(new_capacity > old_capacity + 50 )
			bat ->bat_capacity = new_capacity;
		else
			bat ->bat_capacity = (new_capacity < old_capacity - 25) ? new_capacity : old_capacity;  //avoid the value of capacity increase 
	}


	printk("%s: >>>> last bat_cap = %d\n",__FUNCTION__,  bat ->bat_capacity);
	bat ->bat_change = 1;
}
...
//	struct rk30_adc_battery_data *bat = gBatteryData;
	...
	/*
		if( (bat->resume_time - bat->suspend_time) >= 1800 )
			bat->bat_capacity = (new_capacity+5 < old_capacity) ?  (old_capacity-5) : old_capacity;
		else
...
static int rk30_adc_battery_resume(struct platform_device *dev)
{
	...
	struct rk30_adc_battery_data *data = platform_get_drvdata(dev);
        ...
        if( data->bat_capacity < 15 ){
        	wake_lock_timeout(&batt_wake_lock_detect_lowpower,30*HZ);
        	data->bat_change  = 1;
	}
...
//	pSamples = bat->adc_samples;
	...
	adc_sync_read(bat->client);                             //start adc sample
	first_adc_value = gBatteryData->adc_val ;
	printk(">>>> first_adc_value is  %d \n",first_adc_value);
	level = oldlevel =	 rk_battery_get_status(bat);// rk30_adc_battery_status_samples(bat);//init charge status
	...
	if(0 !=bat ->pdata->low_voltage_protection ){
		...
		{
		     printk(">>>> discharg but battery voltage <= %d ,so system_lowerpower is set 1 \n",BATT_ZERO_VOL_VALUE);
		     system_lowerpower = 1;
	       }else if( ( bat->bat_capacity < 5 ) && (bat->bat_status == POWER_SUPPLY_STATUS_CHARGING) ){
	            printk(">>>> charging but battery capacity < 5 ,so system_lowerpower is set 1 \n");
//	            system_lowerpower = 1; 
	       }
...
#if 1
		if ((bat->bat_voltage <= BATT_ZERO_VOL_VALUE)&&(bat->bat_status != POWER_SUPPLY_STATUS_CHARGING)){
		      printk("boot battery voltage < %dv ,so shutdown \n",BATT_ZERO_VOL_VALUE );
			//kernel_power_off();
			queue_delayed_work(bat ->wq, &bat ->delay_poweroff, msecs_to_jiffies(TIMER_MS_COUNTS*3));
		}
#endif
...
void rk_lowerpower_check(void)
{
	...
	for(i=0; i< NUM_VOLTAGE_SAMPLE; i++){
		...
		check_data[i] = gBatteryData->adc_val;	

		adc_val += check_data[i];
	}
	adc_val /=NUM_VOLTAGE_SAMPLE;
	printk("------------------>> %d,%d <<-----------------\n",adc_val,gBatteryData->adc_value);
	if((adc_val > gBatteryData->adc_value+5) &&(gBatteryData->bat_status == POWER_SUPPLY_STATUS_NOT_CHARGING ) )//
	{
		printk("%d,%d\n",adc_val,gBatteryData->adc_value);
		printk("battery over discharge ,so shutdown \n");
		kernel_power_off();
	}
...
static void rk_adc_battery_delay_poweroff(struct work_struct *work)
{
       kernel_power_off();
}
static int rk30_adc_battery_probe(struct platform_device *pdev)
{
	...
	data->bat_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
 	...
 	wake_lock_init(&charge_display_lock, WAKE_LOCK_SUSPEND, "charge_display_lock");	//charge_display_lock
        wake_lock_init(&batt_wake_lock_detect_lowpower, WAKE_LOCK_SUSPEND, "batt_wake_lock_detect_lowpower");
	...
	INIT_DELAYED_WORK(&data->delay_poweroff, rk_adc_battery_delay_poweroff);
	...
	if( pdata->batt_low_pin != INVALID_GPIO){
		...
		if (gpio_get_value(pdata->batt_low_pin) ==0){
		       mdelay(20);	   
		       if (gpio_get_value(pdata->batt_low_pin) ==0){
		       	 printk("lower power pin warning, so shutdown\n");
		               kernel_power_off(); 
		       }
		}
...
static int rk30_adc_battery_remove(struct platform_device *pdev)
{
    ...
    if (pdata->dc_det_pin != INVALID_GPIO)
        ...
        cancel_delayed_work_sync(&data->delay_poweroff);
...
//subsys_initcall_sync(rk30_adc_battery_init);
